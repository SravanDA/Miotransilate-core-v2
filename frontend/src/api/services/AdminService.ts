...
export const AdminService = {
  getLanguages: async (): Promise<Language[]> => {
    const response = await apiClient.get('/v1/admin/languages');
    return response.data;
  },

  addLanguage: async (payload: any): Promise<Language> => {
    const response = await apiClient.post('/v1/admin/languages', payload);
    return response.data;
  },
  ...
  updateConfig: async (key: string, value: string, etag: number): Promise<SystemConfiguration> => {
    const response = await apiClient.patch(`/v1/admin/config/${key}`, { configValue: value }, {
      headers: {
        'If-Match': `"${etag}"`
      }
    });
