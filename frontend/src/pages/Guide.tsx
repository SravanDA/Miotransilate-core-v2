...
export function Guide() {
 ...
 // Scroll tracking to automatically update the active TOC section
 useEffect(() => {
   const handleScroll = () => {
   ...
   // Listen on window and any scrollable parent container with capture
 ...
 {/* ─── Left sidebar ─── */}
 <aside className="hidden lg:flex flex-col w-56 shrink-0 sticky top-6 self-start max-h-[calc(100vh-6rem)] overflow-y-auto scrollbar-none pr-2">
 <div className="flex items-center gap-2 mb-4 px-3">
 <BookOpen className="w-4 h-4 text-accent-blue" weight="fill" />
 <span className="text-[11px] font-bold text-text-tertiary uppercase tracking-wider">Contents</span>
 </div>
 ...
 className={`text-left px-3 py-2 rounded-lg text-[13px] transition-all cursor-pointer ${
 activeSection === item.id
 ? "bg-accent-blue/10 text-accent-blue font-bold border-l-2 border-accent-blue pl-2.5"
 : "text-text-secondary hover:text-text-primary hover:bg-bg-hover font-medium border-l-2 border-transparent"
 }`}
 >
 {item.label}
 </button>
 ...
 {/* Page header */}
 <div className="mb-6">
 <h1 className="text-xl sm:text-2xl font-bold text-text-primary mb-1">MioTranslate - Team Guide</h1>
 ...
 Written for every team member: product, engineering, support, sales, and localization reviewers.
 </p>

 {/* Mobile / Tablet Quick Navigation Pills */}
 <div className="lg:hidden mt-4 pt-3 border-t border-border-subtle">
 <div className="text-[11px] font-bold text-text-tertiary uppercase tracking-wider mb-2">Jump to Section</div>
 <div className="flex overflow-x-auto gap-1.5 pb-2 scrollbar-none whitespace-nowrap">
 {toc.map((item) => (
 <button
 key={item.id}
 onClick={() => scrollTo(item.id)}
 className={`px-3 py-1 rounded-full text-[12px] font-semibold transition-colors cursor-pointer shrink-0 border ${
 activeSection === item.id
 ...
 MioTranslate organises all translatable content into a simple hierarchy:
      ...
      <SubSection title="Structure: Pages → Tags → Translations">
        <div className="bg-bg-main/50 rounded-lg p-5 border border-border-subtle text-[13px] text-text-primary leading-relaxed">
          <div className="font-bold mb-2">Example:</div>
          <div className="ml-0">
            <div className="flex items-center gap-2 mb-1">
              <FileText className="w-4 h-4 text-accent-blue shrink-0" weight="fill" />
              <span><strong>Page:</strong> Quick Sale (POS screen)</span>
            </div>
            <div className="ml-6 flex flex-col gap-1">
              <div className="flex items-center gap-2">
                <Tag className="w-3.5 h-3.5 text-text-tertiary shrink-0" weight="fill" />
            ...
            <div className="mt-3 ml-12 flex flex-col gap-1 text-text-secondary">
      ...
      <SubSection title="Translation Lifecycle">
        ...
        <div className="flex flex-wrap items-center gap-2 text-[12px] font-semibold mt-2 mb-3">
          <span className="px-3 py-1 rounded bg-bg-hover text-text-tertiary border border-border-subtle">No Translation</span>
          <ArrowRight className="w-3.5 h-3.5 text-text-tertiary" weight="bold" />
          <span className="px-3 py-1 rounded bg-accent-blue/10 text-accent-blue border border-accent-blue/20">Draft</span>
          <ArrowRight className="w-3.5 h-3.5 text-text-tertiary" weight="bold" />
          <span className="px-3 py-1 rounded bg-amber-500/10 text-amber-500 border border-amber-500/20">Pending Review</span>
          <ArrowRight className="w-3.5 h-3.5 text-text-tertiary" weight="bold" />
          <span className="px-3 py-1 rounded bg-emerald-500/10 text-emerald-500 border border-emerald-500/20">Approved</span>
        </div>
        <P>
        ...
        <div className="flex flex-wrap items-center gap-2 text-[12px] font-semibold mt-2 mb-3">
          <span className="px-3 py-1 rounded bg-emerald-500/10 text-emerald-500 border border-emerald-500/20">Approved</span>
          <ArrowRight className="w-3.5 h-3.5 text-text-tertiary" weight="bold" />
          <span className="px-3 py-1 rounded bg-amber-500/10 text-amber-500 border border-amber-500/20">⚠ Stale</span>
          <ArrowRight className="w-3.5 h-3.5 text-text-tertiary" weight="bold" />
          <span className="text-text-secondary font-medium">Re-review or re-translate</span>
        </div>
      </SubSection>
      ...
      <SubSection title="Deployment Pipeline">
        ...
        <div className="flex items-center gap-4 text-[13px] font-bold mt-3 mb-3">
          <span className="px-4 py-2 rounded-lg bg-accent-blue/10 text-accent-blue border border-accent-blue/20">Dev</span>
          <ArrowRight className="w-4 h-4 text-text-tertiary" weight="bold" />
          <span className="px-4 py-2 rounded-lg bg-amber-500/10 text-amber-500 border border-amber-500/20">QA</span>
          <ArrowRight className="w-4 h-4 text-text-tertiary" weight="bold" />
          <span className="px-4 py-2 rounded-lg bg-emerald-500/10 text-emerald-500 border border-emerald-500/20">Production</span>
        </div>
        <BulletList items={[
 ...
 role="PM"
 details={[
 "Go to Pages tab and click '+ Create Page'.",
 ...
 "Click Create. The page now appears in the Pages list with 0 tags.",
 ]}
 />
 ...
 "Optionally select or type a custom copy type (Button, Label, Header, Modal Title, Toast, etc.) to give translators and AI extra UI context.",
 ...
 className="w-full py-4 flex items-start justify-between text-left cursor-pointer hover:bg-bg-hover rounded-lg transition-colors px-2"
 >
 <span className="text-[14px] font-semibold text-text-primary pr-4 leading-snug">{item.q}</span>
 <ChevronRight className={`w-4 h-4 text-text-tertiary shrink-0 mt-0.5 transition-transform ${expandedFaq === idx ? "rotate-90 text-accent-blue" : ""}`} weight="bold" />
 </button>
 {expandedFaq === idx && (
 <div className="px-2 pb-4 text-[13px] text-text-secondary leading-relaxed">
 {item.a}
 </div>
 )}
 </div>
...
function P({ children }: { children: React.ReactNode }) {
 return <p className="text-[13px] text-text-secondary leading-relaxed mb-3 last:mb-0">{children}</p>;
}
...
function BulletList({ items }: { items: string[] }) {
 ...
 <span className="w-1.5 h-1.5 rounded-full bg-accent-blue/60 shrink-0 mt-2"></span>
 <span>{item}</span>
 </li>
 ))}
 </ul>
...
function Callout({ type, children }: { type: "info" | "warning"; children: React.ReactNode }) {
 ...
 const Icon = isInfo ? Lightbulb : AlertTriangle;
 return (
 <div className={`rounded-lg p-4 flex items-start gap-3 text-[13px] leading-relaxed mt-3 border ${
 isInfo 
 ? "bg-accent-blue/10 border-accent-blue/30 text-text-primary" 
 : "bg-warning/10 border-warning/30 text-text-primary"
 }`}>
 <Icon className={`w-4 h-4 shrink-0 mt-0.5 ${isInfo ? "text-accent-blue" : "text-warning"}`} weight={isInfo ? "fill" : "bold"} />
 <div className="leading-relaxed text-text-primary">{children}</div>
 </div>
 );
}
...
function GuideItem({ icon: Icon, title, description }: { icon: any; title: string; description: string }) {
 return (
 <div className="flex items-start gap-4 p-4 rounded-xl bg-bg-main/50 hover:bg-bg-main border border-border-subtle/50 transition-colors">
 <div className="w-10 h-10 rounded-lg bg-accent-blue/10 text-accent-blue flex items-center justify-center shrink-0">
 <Icon className="w-5 h-5 stroke-[1.5]" />
 </div>
 <div>
 <h3 className="text-[14px] font-bold text-text-primary mb-1 leading-tight">{title}</h3>
 <p className="text-[13px] text-text-secondary leading-relaxed m-0">{description}</p>
 </div>
 </div>
 );
}
...
function GlossaryRow({ term, definition }: { term: string; definition: string }) {
 return (
 <div className="py-3 border-b border-border-subtle last:border-b-0 flex items-start gap-4">
 <span className="text-[13px] font-bold text-accent-blue w-40 shrink-0">{term}</span>
 <span className="text-[13px] text-text-secondary leading-relaxed">{definition}</span>
 </div>
 );
}
...
function RoleCard({ code, name, whoIsThis, canDo, cannotDo }: { code: string; name: string; whoIsThis: string; canDo: string[]; cannotDo: string[] }) {
 return (
 <div className="mt-5 first:mt-0 border border-border-subtle bg-bg-main/40 rounded-xl p-5">
 <div className="flex items-center gap-3 mb-3">
 <span className="w-9 h-9 rounded-lg bg-accent-blue/10 text-accent-blue flex items-center justify-center text-[12px] font-bold border border-accent-blue/20">{code}</span>
 <div>
 <h3 className="text-[14px] font-bold text-text-primary leading-tight">{name}</h3>
 <p className="text-[12px] text-text-tertiary m-0">{whoIsThis}</p>
 </div>
 </div>
 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
 <div>
 <span className="text-[11px] font-bold text-success uppercase tracking-wider mb-2 block">Can Do</span>
 <ul className="flex flex-col gap-1.5">
 {canDo.map((item, i) => (
 <li key={i} className="flex items-start gap-2 text-[13px] text-text-secondary">
 <CheckCircle2 className="w-3.5 h-3.5 text-success shrink-0 mt-0.5" weight="fill" />
 <span>{item}</span>
 </li>
 ))}
 </ul>
 ...
 <div>
 <span className="text-[11px] font-bold text-danger uppercase tracking-wider mb-2 block">Cannot Do</span>
 <ul className="flex flex-col gap-1.5">
 {cannotDo.map((item, i) => (
 <li key={i} className="flex items-start gap-2 text-[13px] text-text-secondary">
 <XCircle className="w-3.5 h-3.5 text-danger shrink-0 mt-0.5" weight="fill" />
 <span>{item}</span>
 </li>
 ))}
 </ul>
...
function WorkflowStep({ step, title, role, details }: { step: number; title: string; role: string; details: string[] }) {
 ...
 <span className="px-2 py-0.5 rounded text-[11px] font-bold bg-bg-hover text-text-tertiary border border-border-subtle">{role}</span>
 </div>
 <ul className="flex flex-col gap-1 mb-0">
 {details.map((d, i) => (
 <li key={i} className="flex items-start gap-2.5 text-[13px] text-text-secondary leading-relaxed">
 <span className="w-1.5 h-1.5 rounded-full bg-accent-blue/50 shrink-0 mt-2"></span>
 <span>{d}</span>
 </li>
 ))}
 </ul>
...
function BestPracticeItem({ icon: Icon, title, description }: { icon: any; title: string; description: string }) {
 return (
 <div className="flex items-start gap-4 p-4 rounded-xl bg-bg-main/40 border border-border-subtle/50">
...
function DontItem({ title, description }: { title: string; description: string }) {
 return (
 <div className="flex items-start gap-3.5 p-4 rounded-xl bg-danger/5 border border-danger/20">
 <Ban className="w-4.5 h-4.5 text-danger shrink-0 mt-0.5" weight="fill" />
 <div>
 <h3 className="text-[14px] font-bold text-text-primary mb-0.5 leading-tight">{title}</h3>
 <p className="text-[13px] text-text-secondary leading-relaxed m-0">{description}</p>
 </div>
 </div>
 );
