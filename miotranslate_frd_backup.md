...
- All translations across all supported languages
- Review and approval workflows for copy and translations
- The pipeline from authoring through Dev, QA, and Production
- Version history and audit trail for every label
- Translation coverage and readiness visibility
- Awareness of what tags exist in the MioSalon codebase

**What MioTranslate does not own:**

- The MioSalon codebase (developers continue to write tags in code)
- Language Services (continues to serve tags to MioSalon's UI; MioTranslate only pushes approved content to it)
...
| Capability 11.7 | Codebase Awareness |
...
**Responsibilities.** Creates tags in the MioSalon codebase with fallback English text. Writes the code that references tags. Does not manage UX copy in MioTranslate.

**Goals.** See the status of tags they created. Understand whether their tags have been authored and translated.

**Primary workflows.** View pages and tags. Check tag status. Notify MioTranslate of new tags (developer notification flow).

**Permissions.** View-only. View all pages, tags, statuses, and history. Submit developer notifications. Cannot author, edit, translate, approve, or publish.
...
| Module | The MioSalon module this page belongs to (POS, CRM, Calendar, Reporting, Settings, Staff). Metadata only, not an organizational layer. |
| Tag Count | Total number of tags on this page |
| Status | Active or Deprecated |
| Created Date | When this page was first registered in MioTranslate |
| Last Sync Date | When this page was last synced with the codebase |

**Lifecycle.** A page is created when discovered during sync or migration. It remains Active as long as it exists in the MioSalon codebase. If all tags on a page are removed from the codebase, the page is marked Deprecated. A deprecated page is not deleted; it remains for historical reference.
...
| Attribute | Description |
|---|---|
| Tag ID | Unique identifier matching the codebase (e.g., QUICK_1, QUICK_42, CUSWISH_FEMALE) |
| Page ID | The page this tag belongs to |
| Developer Fallback | The hardcoded English text written by the developer in the codebase |
| Copy Type | The type of UX element (button, label, error message, header, placeholder, helper text, status badge, filter option, table header, validation message). Optional, set by the author. |
| Status | Active or Deprecated |
| Created Date | When this tag was first registered in MioTranslate |

**Lifecycle.** A tag is created when discovered during sync, developer notification, or migration. It remains Active while referenced in the codebase. If removed from the codebase, it is marked Deprecated. Deprecated tags are excluded from active workflows but retained for historical reference.
...
1. **No Copy** : Tag exists but no English copy has been authored yet. The developer's fallback is the only reference.
...
### 4.9 Release (Promotion)

**Purpose.** The act of moving a page bundle (one page + one language) from one environment to the next.

**Attributes.**

| Attribute | Description |
|---|---|
| Page ID | The page being promoted |
| Language | The language being promoted |
| Version | The version being promoted |
| Source Environment | Where it was (e.g., Dev) |
| Target Environment | Where it is going (e.g., QA) |
| Promoted By | Who initiated the promotion |
| Approved By | Who approved the promotion |
| Promotion Date | When |
| Tag Count | Number of tags in the promoted bundle |
| Status | Successful, Failed, Rolled Back |

**Lifecycle.** A release is created when a promotion is initiated. It is marked Successful when the content is confirmed in the target environment. It can be marked Rolled Back if a subsequent rollback occurs.

---

### 4.10 Environment

**Purpose.** Represents a stage in the publishing pipeline.
...
| Current State | Per page per language: which version is currently deployed |

**Lifecycle.** Environments are fixed. They are not created or removed by users.

**Business rule.** Content must progress through environments in order: Dev, then QA, then Production. Content cannot skip an environment.
...
| Attribute | Description |
|---|---|
| Action | What happened (created, edited, approved, rejected, promoted, rolled back, synced, etc.) |
...
**Purpose.** A record of a synchronization between MioTranslate and the MioSalon codebase.

**Attributes.**

| Attribute | Description |
|---|---|
| Initiated By | Who triggered the sync (user or system) |
| Date | When |
| New Pages Detected | Count |
| New Tags Detected | Count |
| Changed Fallbacks Detected | Count |
| Deprecated Tags Detected | Count |
| Status | Completed, Failed |

**Lifecycle.** A sync event is created when a sync is triggered and completed when processing finishes.
...
**Purpose.** The foundation of MioTranslate. Maintains an accurate registry of every page and tag in MioSalon.

**Business context.** Traces to Capability 11.7 (Codebase Awareness) and Objective 1 (single source of truth). If MioTranslate does not know a tag exists, that tag cannot be managed.

**Responsibilities.**

- Register pages and tags discovered through sync or migration
- Track the developer's fallback text for each tag
- Detect new pages, new tags, changed fallbacks, and deprecated tags
- Provide browsing and search across all pages and tags

**Primary features.**

- Page listing with tag counts, module, and status
- Tag listing within a page with fallback text, copy status, and translation status per language
- Sync-on-demand with diff summary (new, changed, deprecated)
- Developer notification flow (developer submits known additions)
...
- Sync compares the codebase registry against MioTranslate's registry and surfaces differences
...
- Track the developer's fallback separately from the approved English copy
- Version every change to English copy
- Flag all translations as Stale when English copy changes

**Primary features.**

- Author English copy for a tag (new or replacing fallback)
- Edit existing English copy
- View developer fallback alongside approved copy
- View English copy version history
- Flag divergence between developer fallback and approved copy
- Submit English copy for review
- Change reason capture (optional)

**Business rules.**

- The developer's fallback text is never overwritten in MioTranslate. It is stored as a reference.
- The approved English copy is independent of the fallback. It may match exactly, or it may be completely different.
...
**Dependencies.** Page & Tag Registry (tags must exist before English copy can be authored).
...
- Manage the Dev, QA, Production pipeline
- Enforce environment sequence (no skipping)
- Deploy page bundles (one page + one language) to environments
- Track what is deployed where
- Support rollback to a previous version
- Record every promotion and rollback

**Primary features.**

- View environment status per page per language (what version is deployed where)
- Promote a page bundle to the next environment
- Approval gate at each promotion step
- Rollback a page bundle to a previous version in any environment
- Deployment history per page per language
- Pre-promotion summary: what is changing compared to what is currently deployed

**Business rules.**

- The unit of deployment is a page bundle: one page + one language
- Promotion sequence is fixed: Dev, then QA, then Production. No environment can be skipped.
- Promotion requires approval. The required approver depends on the target environment:
  - Dev: Author or Reviewer
  - QA: Reviewer
  - Production: Support Reviewer or Founder
- Only approved content can be promoted. Draft or Pending Review items cannot be included in a bundle.
- Promoting a page bundle pushes all approved tags for that page in that language to the target environment.
- Promoting one language does not affect other languages for the same page.
- Promoting one page does not affect other pages.
- Rollback re-deploys the previous version. The rolled-back version is not deleted; it remains for investigation.
- Every promotion and rollback is recorded with who, when, what version, source environment, target environment.
...
**Purpose.** Detect changes in the MioSalon codebase and update MioTranslate's registry accordingly.

**Problem solved.** If MioTranslate does not know a tag exists, it cannot be managed. (Capability 11.7)

**Primary users.** Product Manager, Administrator.

**Preconditions.** MioTranslate has been initialized (migration complete or first sync run).

**Functional behaviour.**

- User triggers sync from within MioTranslate
- MioTranslate reads all pages and tags from the codebase registry (Language Services)
- MioTranslate compares the codebase registry against its own registry
- MioTranslate generates a diff summary

**System behaviour.**

- New pages: registered with all their tags. Each tag status: "Needs English copy."
- New tags on existing pages: registered. Existing tags unaffected.
- Changed fallbacks: flagged for review. The approved English copy may need updating.
- Removed tags: marked Deprecated. Excluded from active workflows but retained.

**User actions after sync.**

- Review the diff summary
- Drill into any page to see specific changes
- Author English copy for new tags
- Review changed fallbacks
- Acknowledge deprecated tags

**Business rules.**

- Sync does not modify existing approved English copy or translations
- Sync only adds or flags; it never deletes
- A sync event is recorded in the audit trail

**Edge cases.**

- A sync finds zero changes: display "No changes detected" with the sync timestamp
- A sync finds a tag that was previously deprecated but is now back in the codebase: reactivate the tag, preserve its history
- A page's name changes in the codebase but its Page ID remains the same: update the name, log the change

**Acceptance criteria.**

- After sync, every page and tag in the codebase is represented in MioTranslate
- New tags are surfaced in the "Needs English copy" state
- Changed fallbacks are flagged for review
- Removed tags are marked Deprecated
- A sync event record is created in the audit trail

---

### F-02: Developer Notification

**Purpose.** Allow a developer to notify MioTranslate of known additions without waiting for a full sync.

**Problem solved.** Urgency: developer ships a feature and wants the product team to see the new tags immediately. (Capability 11.7)

**Primary users.** Developer.

**Preconditions.** Developer has MioTranslate access (view-only role or above).

**Functional behaviour.**

- Developer submits a notification: "I added N tags to page X"
- MioTranslate pulls just that page from the codebase registry and registers new tags

**Business rules.**

- Notification triggers a targeted sync for the specified page only
- Other pages are not affected
- The notification is recorded in the audit trail

**Acceptance criteria.**

- New tags on the specified page are registered in MioTranslate within minutes of notification
- The developer sees confirmation that the tags were registered

---

### F-03: Author English Copy
...
**Preconditions.** Tag exists in MioTranslate (registered through sync or migration).

**Functional behaviour.**

- User navigates to a tag
- User sees the developer's fallback text as reference
...
- The developer's fallback text is displayed but never overwritten
...
- Author writes English copy that exactly matches the developer's fallback: this is valid. The match is noted but the copy still goes through the review process.
...
### F-06: Translate All (Bulk AI Translation)
...
### F-08: Bulk Approve Translations
...
### F-09: Resolve Stale Translation
...
### F-10: Promote Page Bundle

**Purpose.** Move all approved tags for a page in a specific language to the next environment.
...
**Preconditions.** Page bundle has approved content in the current environment. Required approver for the target environment has authority.

**Functional behaviour.**

- User selects a page and language
- User sees the current environment status (e.g., "QUICK Arabic is in Dev, ready for QA")
- User sees a pre-promotion summary: what tags are included, what changed since the last deployment to the target environment
- User initiates promotion
- Required approver approves the promotion
- Content is pushed to the target environment

**Business rules.**

- Promotion follows a fixed sequence: Dev to QA to Production
...
- Promoting one page+language does not affect other pages or other languages
- Every promotion is recorded with who, when, what version, and a tag count

**Edge cases.**

- A tag on the page is still in Draft while promotion is initiated: that tag is excluded from the bundle. The user is shown the count of excluded tags.
- All tags on the page are in Draft: promotion cannot proceed. Display message: "No approved content to promote."
- Promotion fails (target environment unreachable): record the failure. Content remains in the source environment. User can retry.

**Acceptance criteria.**

- Promotion follows the fixed environment sequence
- Approval is required at each step
- Pre-promotion summary shows what will change
- Every promotion is recorded in deployment history
...
- Rollback does not require re-promotion through earlier environments. It directly restores the selected version in the target environment.
...
### F-14: Filter by Translation State
...
- Every audit record includes the actor, timestamp, action, subject, and details.
...
- Chronological list of recent actions: edits, approvals, promotions, syncs
- Filterable by user, page, language, action type
- Shows the most recent actions first

---

### F-20: Migration Import

**Purpose.** One-time import of all existing pages, tags, and translations from Language Services into MioTranslate.

**Problem solved.** MioTranslate must start with the current state of the product. (Objective 1)

**Primary users.** Administrator.

**Preconditions.** This is a one-time operation before MioTranslate goes live.

**Functional behaviour.**

- All pages, tags, and existing translations are imported from Language Services
- Imported content enters MioTranslate as "Published in Production" (it is already live)
- After migration, a validation report is generated comparing MioTranslate's state against the source

**Post-migration actions.**

- Developer write access to Language Services is revoked
- All future UX copy changes go through MioTranslate

**Business rules.**

- Migrated content is not treated as Draft. It is the current production state.
- Migration preserves all existing translations as-is
- A full snapshot of the source data is taken before migration begins (safety)
- Post-migration validation confirms nothing was lost or corrupted

**Acceptance criteria.**

- Every page, tag, and translation in Language Services is represented in MioTranslate after migration
...
4. The developer's fallback text and the approved English copy are tracked separately. Both are preserved.
5. When English copy changes, all translations of that tag across all languages are automatically flagged Stale.
6. A Stale flag does not remove the existing deployed translation. It is advisory.
...
11. Bulk approval is available only for translations meeting the configurable confidence threshold.
12. Approval is required at every environment promotion step.
13. Promotion to Production requires Support Reviewer or Founder approval.

### Publishing Rules

14. The unit of deployment is a page bundle: one page + one language.
15. Content progresses through environments in order: Dev, QA, Production. No environment can be skipped.
16. Only one version of a page bundle can be active in an environment at a time.
17. Rollback restores a previous version without deleting the rolled-back version.
18. Every promotion and rollback is recorded.
...
| Submit developer notification | Yes | No | No | No | No | No |
| Trigger sync | No | Yes | No | No | Yes | Yes |
...
1. Developer adds a new tag in the MioSalon codebase with fallback English text
2. MioTranslate discovers the tag during sync (or developer notification)
3. Tag is registered. Status: Needs English copy.
4. PM/QA reviews the developer's fallback and authors the official English copy
5. English copy is submitted for review
6. Reviewer approves the English copy
7. PM or Localization Reviewer triggers AI translation for selected languages
8. AI generates translations with back-translations and confidence scores
9. Localization Reviewer reviews each translation
10. Localization Reviewer approves (or edits and approves)
11. Approved content is promoted to Dev
12. QA verifies the labels in context on the Dev build
13. Content is promoted to QA
14. Support team verifies the labels
15. Content is promoted to Production
16. Salon teams see the labels in their language
...
### WF-09: Sync and Handle New Tags

1. PM triggers "Sync"
2. MioTranslate compares codebase registry with its own
3. Diff summary shows: 2 new tags on QUICK, 1 changed fallback on INVOICE, 1 new page LOYALTY with 15 tags
4. PM authors English copy for the 2 new QUICK tags
5. PM reviews the changed fallback on INVOICE
6. PM begins English copy authoring for the new LOYALTY page
...
1. Developer removes a tag from the MioSalon codebase
2. Next sync detects the tag is no longer referenced
3. Tag is marked Deprecated in MioTranslate
4. Tag is excluded from active workflows (no further translation or promotion)
...
| Tag ID | Immutable after creation. Must match the codebase. |
| Page ID | Immutable after creation. Must match the codebase. |
...
| Environment sequence | Cannot promote to QA without first deploying to Dev. Cannot promote to Production without first deploying to QA. |
...
|---|---|---|
| New tags discovered during sync | PM, QA | New tags need English copy |
...
| Translation approved | PM | Translation is ready for promotion |
...
| Page bundle promoted to Production | PM, Support Reviewer | Labels are now live |
| Rollback initiated | PM, QA, Support Reviewer | A previous version has been restored |
| Promotion failed | PM, Administrator | Action required: investigate and retry |
| Developer notification received | PM | New tags registered; English copy needed |
| Sync completed with changes | PM | New tags, changed fallbacks, or deprecations detected |
...
| **Reliability** | MioTranslate must be consistently available to the internal team during working hours. A failure in MioTranslate must not affect MioSalon's ability to serve UX copy to salon teams (Language Services continues to function independently). |
...
| **Data Safety** | No operation in MioTranslate should corrupt or overwrite data in Language Services. Pushes to Language Services should be atomic: either fully successful or not applied. |

---

## 15. Assumptions

1. Language Services will remain operational and continue to serve MioSalon's UI. MioTranslate depends on Language Services as the delivery mechanism.
2. The tag naming convention (PAGE_ID_TAG_NUMBER) will remain consistent in the codebase.
...
6. The number of environments is fixed at three: Dev, QA, Production.
...
1. **No changes to MioSalon's rendering.** MioSalon's tag system, Language Services API, and how MioSalon displays UX copy remain unchanged.
...
4. **Developer write access to Language Services will be revoked.** After migration, MioTranslate is the only system that writes to Language Services.
...
|---|---|---|
| What is the exact confidence threshold for bulk approval? | Business rule 11 states bulk approval requires a configurable threshold. The default needs to be set. | Affects review workflow efficiency. |
| Which pages or labels require Founder-level approval? | Some sensitive screens may always need Founder sign-off. The list needs to be defined. | Affects governance configuration. |
| How should MioTranslate handle tags shared across multiple pages? | Some labels (e.g., "Save", "Cancel") may appear on many pages. Should they be managed once or per page? | Affects content management strategy. Needs investigation. |
| What is the expected response time for AI translation? | Bulk translation of 100+ tags may take time. What is acceptable? | Affects UX expectations for bulk operations. |
| Should the Dev environment promotion be implicit or explicit? | When English copy or translation is approved, does it automatically appear in Dev, or does someone need to explicitly promote to Dev? | Affects workflow efficiency. |
| How will the support team's approval workflow integrate with their existing processes? | The support team currently approves informally. How do they want the formalized process to work? | Affects role definition and workflow design. |
...
1. **Automated sync** (triggered on code merge or daily schedule) to replace sync-on-demand
2. **Translation memory** (reuse previously approved translations for identical or similar English copy across pages)
3. **Terminology glossary** (define standard translations for key terms like "appointment," "invoice," "walk-in" across all languages)
4. **Real-time collaborative editing** (if the team grows significantly)
5. **Translation rules engine** (formalized rules governing tone, terminology, and context for each language)
...
7. **Integration with CI/CD pipeline** (trigger sync automatically when code is merged)
